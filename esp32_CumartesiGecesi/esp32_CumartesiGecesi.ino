  ...
  for (;;) {
    //if (telemetry_state == true) {
      ...
      if (temperature != prevTemperature || pressure != prevPressure || altitude != prevAltitude) {
        ...
        // Önceki değerlerle aynı, BMP280 sensörünü yeniden başlat
        bmp.begin(0x76);
      }
    //}
    // else {
    //   Serial.println("1- Error BMP280--> Simulation Mode");
    // }
...
void TaskIMU(void* pvParameters) {
  ...
  for (;;) {
    ...
    Serial.println("4- IMU--> Rotation X: ");
...
void setup() {
  ...
  SerialGPS.begin(9600, SERIAL_8N1, 16, 27);  // RX - TX
