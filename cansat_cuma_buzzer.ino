...
float current_altitude = 0.0;      // bulunduğunuz yerin yüksekliği (metre)
float total_altitude =0.0 ; 
...
bool telemetry_switch = false ; // true -> On , false -> Off
  ...
  for (;;) {
      //  if (telemetry_state == true) {
        ...
        if (temperature != prevTemperature || pressure != prevPressure || altitude != prevAltitude) {
          TelemetryObject.temp = String(temperature,1);
          TelemetryObject.pressure = String(pressure);
          TelemetryObject.altitude = String(altitude - current_altitude);
    ...
    for (;;) {
        ...
        if (XBee.available()) {
            ...
            Serial.println("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!Gelen Ham Veri -->  " + incoming_data + "   ;  " + "Boyutu -->  " + incoming_data.length() + "   ;  " + "Son -2 Char -->" + incoming_data[incoming_data.length() - 1]);
                ...
                {
                    digitalWrite(32, HIGH); // D13 pimini yüksek seviyeye ayarlama
                ...
                {
                    digitalWrite(32, LOW); // D13 pimini yüksek seviyeye ayarlama
                ...
                else if (incoming_data == "SERVOOFF")  // 33
                {
                    servo.write(175);
                    Serial.println("/////SERVO");
                    Serial.println();
                ...
                else if (incoming_data.substring(0, 12) == "CMD,1007,ST," )
                {
                    String set_mission = incoming_data.substring(12, incoming_data.length()-1); // bu satır kontrol edilecek
                    TelemetryObject.mission_time= set_mission;       // Fake pressure
                    Serial.println("Missinon time set");
                }

                else if (incoming_data == "CMD,1007,CAL")
                {
                          current_altitude = total_altitude/altitude_counter;
                          altitude_counter=0;
                          total_altitude =0;
                }
                ...
                else if (incoming_data.substring(0, 14) == "CMD,1007,SIMP," && telemetry_state == false)
                {
                    ...
                    float float_command= command.toFloat();
                    if (telemetry_state == false) {
                        float fake_altitude =44330.0 * (1.0 - pow(  float_command / seaLevelPressure, 0.1903));
                        TelemetryObject.pressure= command;       // Fake pressure          
                        TelemetryObject.altitude= fake_altitude;
                        TelemetryObject.temp=bmp.readTemperature();
                        Serial.println("FAKE- BMP280--> " + TelemetryObject.pressure + "," + TelemetryObject.temp + "," + TelemetryObject.altitude);
                    }
...
void TaskRTC(void* pvParameters) {
  ...
  for (;;) {
    ...
    Serial.println("10- RTC WORKING --> ");

    DateTime now = rtc.now();
    TelemetryObject.mission_time = String(now.hour(), DEC) + ":" + String(now.minute(), DEC) + ":" + String(now.second(), DEC);
    vTaskDelay(800);
